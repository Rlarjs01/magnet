...
namespace SimplePhysicsToolkit {
	public class MagnetOFF : MonoBehaviour {
		...
		private float magnetForce2;

		void Start() {
			StartCoroutine(OFFma());
		}
        ...
        void attractOrRepel(Collider col){// 끌꺼나 민다
				if (!useColliderAsTrigger)
				{
		...
		IEnumerator OFFma(){
			...
			{
			if (magnetForce != 0)
			{magnetForce2 = magnetForce;
			yield return new WaitForSeconds(OFFcount);
			 magnetForce = 0;
			}
			else if(magnetForce == 0){
				yield return new WaitForSeconds(OFFcount);
				magnetForce = magnetForce2;
			}
			}
		}
	}
